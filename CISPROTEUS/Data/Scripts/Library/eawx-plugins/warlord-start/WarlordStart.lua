...
function WarlordStart:Populate_Chosen_Faction(choice, cosmetic)
	...
	if choice == "YEVETHAN_PROTEUS" then
		...
		for _, structure in pairs(entry.SwapStructures) do
			...
			if choice == "YEVETHAN_PROTEUS" then
				if string.find(structure, "Imperial_Proteus_") then
					...
					new_structure_string = "Yevetha_"..partial_structure_string
				...
				elseif string.find(structure, "I_Ground_") then
					...
					new_structure_string = "Y_Ground_"..partial_structure_string
					local oldlist = Find_All_Objects_Of_Type(structure)
					for _, old in pairs(oldlist) do
						local planet = old.Get_Planet_Location()			
						local owner = old.Get_Owner()
	...
	if entry.RemoveUnits then
		for _, unit in pairs(get_value_per_era(entry.RemoveUnits, year)) do
			if Find_First_Object(unit) ~= nil then
				local exists = Find_All_Objects_Of_Type(unit)
				for _, object in pairs(exists) do
					if object.Get_Owner() == Find_Player(self.warlord_name) then
						object.Despawn()
					end
				end
			end
	...
	if entry.SpaceUnitLimit then
		for _, planet in pairs(get_value_per_era(entry.Planets, year)) do
			if FindPlanet(planet) ~= nil then
				local locale = FindPlanet(planet)
				local unit_table_counter = {}
				local all_fleets = Find_All_Objects_Of_Type("Galactic_Fleet")
				for _, fleet in pairs(all_fleets) do
					if fleet.Get_Parent_Object() == locale then
						local planet_units = Find_All_Objects_Of_Type(self.warlord_player)
						for _, unit in pairs(planet_units) do
							if unit.Get_Parent_Object() == fleet then
								local unit_type = unit.Get_Type().Get_Name()
								if unit_table_counter == {} or unit_table_counter[unit_type] == nil then
									unit_table_counter[unit_type] = 1
								else
									unit_table_counter[unit_type] = unit_table_counter[unit_type] + 1
								end
							end
						end
					end
				...
				if unit_table_counter ~= {} then
					for unit_name, _ in pairs(unit_table_counter) do
						if unit_table_counter[unit_name] >= entry.SpaceUnitLimit then
							local remove_count = unit_table_counter[unit_name] - entry.SpaceUnitLimit
							local random = GameRandom.Free_Random(1, 10)

							if random <= 2 then
								remove_count = remove_count + 2
							elseif random <= 6 then
								remove_count = remove_count + 1
							end

							if remove_count > unit_table_counter[unit_name] then
								remove_count = unit_table_counter[unit_name]
							end

							for i = remove_count, 1, -1 do
								local all_unit_type_instance = Find_All_Objects_Of_Type(unit_name)
								for _, object in pairs(all_unit_type_instance) do
									if object.Get_Planet_Location() == locale then
										object.Despawn()
										break
									end
								end
	...
	if entry.SwapInfantry then
		for _, unit in pairs(get_value_per_era(entry.SwapInfantry, year)) do
			local oldlist = Find_All_Objects_Of_Type(unit)
			for _, old in pairs(oldlist) do
				local planet = old.Get_Planet_Location()			
				local owner = old.Get_Owner()
				if owner == Find_Player("local") then
					old.Despawn()				
					local spawn_list_new = { "Yevethan_Soldier_Company" }
					local dummies = SpawnList(spawn_list_new, planet, owner, false, false)			
				end
			end
		end
