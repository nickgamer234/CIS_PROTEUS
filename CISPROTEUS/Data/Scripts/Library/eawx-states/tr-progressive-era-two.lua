    ...
    on_enter = function(self, state_context)
        ...
        if self.entry_time <= 5 then
            if Find_Player("local") == Find_Player("Rebel") then
                StoryUtil.Multimedia("TEXT_CONQUEST_NR_INTRO_E2", 15, nil, "Mon_Mothma_Loop", 0)
			...
			elseif Find_Player("local") == Find_Player("Hutt_Cartels") then
                StoryUtil.Multimedia("TEXT_CONQUEST_HUTTS_ZORBA_INTRO", 15, nil, "Zorba_Loop", 0)
                Story_Event("HUTTS_ZORBA_STARTED")
            end
